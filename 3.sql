...
-- Q1: Retrieve movie titles directed by 'HITCHCOCK'
SELECT MOV_TITLE
FROM MOVIES
WHERE DIR_ID IN (
    SELECT DIR_ID 
...
-- Q2: Retrieve movie titles in which an actor has acted in more than one movie
SELECT M.MOV_TITLE
FROM MOVIES M
...
-- Q3: Retrieve actors who acted in at least one movie before 2000 and after 2015
SELECT A.ACT_NAME, M.MOV_TITLE, M.MOV_YEAR
FROM ACTOR A
JOIN MOVIE_CAST C ON A.ACT_ID = C.ACT_ID
JOIN MOVIES M ON C.MOV_ID = M.MOV_ID
WHERE A.ACT_ID IN (
    -- Select actors who acted in a movie before 2000
    SELECT C1.ACT_ID
    FROM MOVIE_CAST C1
...
AND A.ACT_ID IN (
    -- Select actors who acted in a movie after 2015
    SELECT C2.ACT_ID
    FROM MOVIE_CAST C2
...
-- Q4: Retrieve movie titles with their highest review stars, ordered by movie title
